...
public partial class ProjectDetailPage : ContentPage
{
    ...
    private async void OnImportFromGalleryClicked(object? sender, EventArgs e)
    {
        try
        {
            var options = new MediaPickerOptions { Title = "Select photos to import" };
            var results = await MediaPicker.PickMultipleAsync(options);
        ...
        {
            await DisplayAlertAsync("Not Supported", "Photo gallery access is not available on this device.", "OK");
        }
        catch (Exception ex)
        {
